...
<h2  style="color:green; text-align:center;">Video → After Complete → Show Image</h2>

### 💡 Approach

* Use `useState` to track whether video is finished
* Use `onEnded` event from `<video>`
* After video completes → switch to `<img>`
...
* Ads (video ad → banner image)
* App intro (intro video → landing screen)
* Story feature (like reels preview → cover image)
---

<h2  style="color:green; text-align:center;">Show a loader until API data loads</h2>

👉 When the component loads:

* Show **loading spinner/text**
* Call API
* After data comes → hide loader and show data
...
---
<h2  style="color:green; text-align:center;">Permanent disable until the component is re-rendered or the state is manually reset.</h2>
...
function App() {
  // 1. Create a state to track if the button is disabled
  ...
  return (
    <div>
      {/* 3. Bind the state to the 'disabled' property */}
      <button 
        onClick={handleClick} 
        disabled={isDisabled}
      >
...
<h2  style="color:green; text-align:center;">Form validation (required fields)</h2>
...
<h2  style="color:green; text-align:center;">Load image with fallback if broken</h2>
...
<h2  style="color:green; text-align:center;">Debounce search input (API optimization)</h2>

## ✅ Simple Code Example (using `setTimeout`)
...
function SearchComponent() {
  ...
  useEffect(() => {
    if (debouncedQuery) {
      ...
      // example API
      fetch(`https://jsonplaceholder.typicode.com/users?name=${debouncedQuery}`)
        .then(res => res.json())
